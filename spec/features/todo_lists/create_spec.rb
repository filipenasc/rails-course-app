...
describe "Creating todo lists" do
	it "redirects to the todo list index on success" do
		...
		fill_in "Description", with: "loucura loucura"
	...
	it "displays an error when todo list has no title" do
		...
		expect(TodoList.count).to eq(0)
		visit "/todo_lists"
		expect(page).to_not have_content("loucura loucura")
	end
	...
	it "displays an error when todo list has no description" do
		...
		fill_in "Title", with: "Lista marota"
		...
		expect(TodoList.count).to eq(0)
		visit "/todo_lists"
		expect(page).to_not have_content("Lista marota")
	end
